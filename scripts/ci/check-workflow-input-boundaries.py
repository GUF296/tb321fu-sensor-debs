...
def main() -> None:
    ...
    text = "\n".join(lines)
    required = (
        "INPUT_SENSOR_DEB_VERSION: ${{ inputs.sensor_deb_version }}",
        '[[ "$INPUT_SENSOR_DEB_VERSION" =~ ^[0-9][0-9A-Za-z.+~_-]{0,63}$ ]]',
        'dpkg --validate-version "$INPUT_SENSOR_DEB_VERSION"',
        "printf 'SENSOR_DEB_VERSION=%s\\n' \"$INPUT_SENSOR_DEB_VERSION\"",
    )
